...
import {
    ...
    initializeModals,
    openModal
} from '../../tds/tds-previous/tds.js';
import {
    sendDebugMessage,
    showElement,
    hideElement
} from '../../utils/AvayaChatUtils.js';
import FormHelper from '../../helpers/tds-previous/FormHelper.js';
...
class AvayaChatUserInterface {
     ...
     */
    handleBubbleClick() {
        hideElement('avaya-chat__button-badge');
    ...
    generateAvayaChatButton() {
        ...
        avayaChatSvg.classList.add(`tw-chat--${this.chatPrefix}__image`);

        const avayaChatSvgContainer = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        avayaChatSvgContainer.setAttribute('fill', '#171A20');

        const avayaChatSvgPathOne = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        avayaChatSvgPathOne.setAttribute(
            'd',
            'M23.8.85H4.2A3.85 3.85 0 00.35 4.7v12.6l.007.235A3.85 3.85 0 004.2 21.15h3.15v2.859c0 .332.117.653.33.907l.122.13c.515.478 1.317.51 1.87.047l4.709-3.943H23.8a3.85 3.85 0 003.85-3.85V4.7A3.85 3.85 0 0023.8.85zM4.2 2.95h19.6c.966 0 1.75.784 1.75 1.75v12.6a1.75 1.75 0 01-1.75 1.75h-9.927a.7.7 0 00-.45.163L9.45 22.54v-2.79a.7.7 0 00-.7-.7H4.2a1.75 1.75 0 01-1.75-1.75V4.7c0-.966.784-1.75 1.75-1.75z'
        );

        const avayaChatSvgPathTwo = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        avayaChatSvgPathTwo.setAttribute(
            'd',
            'M14 9.6a1.4 1.4 0 110 2.8 1.4 1.4 0 010-2.8zm-5.6 0a1.4 1.4 0 110 2.8 1.4 1.4 0 010-2.8zm11.2 0a1.4 1.4 0 110 2.8 1.4 1.4 0 010-2.8z'
        );

        const avayaChatLoadingSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        AvayaChatUserInterface.setAttributes(avayaChatLoadingSvg, {
            viewBox: '0 0 100 100',
            xmlns: 'http://www.w3.org/2000/svg',
        });
        avayaChatLoadingSvg.classList.add(`tw-chat--${this.chatPrefix}__loading-image`);

        const avayaChatLoadingSvgCircle = document.createElementNS(
            'http://www.w3.org/2000/svg',
            'circle'
        );
        AvayaChatUserInterface.setAttributes(avayaChatLoadingSvgCircle, {
            cx: '50',
            cy: '50',
            r: '45',
        });
        avayaChatLoadingSvgCircle.classList.add('tw-chat--avaya-chat__loading-circle');

        const avayaChatBadge = document.createElement('div');
        avayaChatBadge.classList.add('tw-chat--avaya-chat__button-badge');
        avayaChatBadge.setAttribute('id', 'avaya-chat__button-badge');
        avayaChatBadge.style.display = 'none';

        avayaChatSvgContainer.append(avayaChatSvgPathOne);
        avayaChatSvgContainer.append(avayaChatSvgPathTwo);
        avayaChatSvg.append(avayaChatSvgContainer);

        avayaChatLoadingSvg.append(avayaChatLoadingSvgCircle);
        avayaChatButton.append(avayaChatSvg);
        avayaChatButton.append(avayaChatLoadingSvg);

        avayaChatButton.append(avayaChatBadge);
    ...
    generateAvayaChatForm() {
        ...
        avayaChatForm.classList.add(`tw-chat--${this.chatPrefix}__modal-form`);
        ...
        for (let i = 0; i < fields.length; i++) {
            if (fields[i].type === 'InputItem') {
                ...
                if (fields[i].attributes.name === 'phone') {
                    defaultValue =
                        fields[i].attributes.name in formPrefilledValues ?
                        formPrefilledValues[fields[i].attributes.name].number :
                        null;
                }
    ...
    generateStatusPopUp() {
        ...
        avayaCanvasTray.style.position = 'absolute';
    ...
    generateAvayaChatFormItem(
        ...
        if (type === 'text' || type === 'email') {
            ...
            tdsTextInputWrapper.classList.add('tw-chat--tds-form-input-wrap');

            const tdsTextInput = document.createElement('input');
            tdsTextInput.classList.add('tw-chat--tds-form-input');
        ...
        if (type === 'select') {
            ...
            tdsSelect.classList.add('tw-chat--tds-form-input-wrap');

            const tdsSelectInput = document.createElement('select');
            tdsSelectInput.classList.add('tw-chat--tds-form-input');
    ...
    static generateAvayaChatFormCheckboxItem(name, label, options, required) {
        ...
        fieldSet.classList.add('tw-chat--tds-fieldset', 'tw-chat--tds-fieldset--choice');
        ...
        legendElement.classList.add('tw-chat--tds-legend');
        ...
        options.forEach((option) => {
            ...
            formWrapper.classList.add('tw-chat--tds-form-item', 'tw-chat--tds-form-item--choice');

            const checkboxInput = document.createElement('input');
            checkboxInput.setAttribute('type', 'checkbox');
            checkboxInput.setAttribute('id', option.split(' ').join(''));
            checkboxInput.setAttribute('name', option);
            checkboxInput.classList.add('tw-chat--tds-form-input');

            formWrapper.appendChild(checkboxInput);

            const labelWrapper = document.createElement('div');
            labelWrapper.classList.add('tw-chat--tds-form-label-wrap');

            const checkboxLabel = document.createElement('label');
            checkboxLabel.classList.add('tw-chat--tds-form-label');
            checkboxLabel.setAttribute('for', option.split(' ').join(''));

            const labelSpan = document.createElement('span');
            labelSpan.classList.add('tw-chat--tds-form-label-text', 'tw-chat--tds-text--600');
            labelSpan.innerHTML = DOMPurify.sanitize(option);

            checkboxLabel.appendChild(labelSpan);
            labelWrapper.appendChild(checkboxLabel);
            formWrapper.appendChild(labelWrapper);
            fieldSet.appendChild(formWrapper);
        });
        ...
        checkboxFeedback.classList.add('tw-chat--tds-form-feedback-feedback');
    ...
    generateAvayaChatFrame() {
        ...
        footerTextInputWrapper.classList.add('tw-chat--tds-form-input-wrap');
        ...
        footerTextInput.classList.add('tw-chat--tds-form-input');
    ...
    init() {
        ...
        avayaChat.classList.add(`tw-chat--${this.chatPrefix}-previous`);
    ...
    getAvayaChatVehicleDeliveryFAQ() {
        ...
        avayaChatHeadingDescription.innerHTML = DOMPurify.sanitize(
            this.avayaChatConfig.triageConfig.vehicleDeliveryDesription
        );
        ...
        avayaChatVehicleDeliveryLink.href = this.avayaChatConfig.triageConfig.vehicleDeliveryLoginLink;
    ...
    generateFAQLinks(containerName, associateLinks, needHelpUrl = null) {
        ...
        Object.entries(associateLinks).forEach(([key, value]) => {
            ...
            anchorElement.innerText = key;

            anchorElement.addEventListener('click', () => {
                this.avayaAnalyticsHelper.fireEvent(this.avayaAnalyticsHelper.supportInteraction, value);
            });
